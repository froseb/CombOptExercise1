...
/**
   Nodes in DIMACS files are counted from 1, but here we count them from 0 so
they match their std::vector indices. These two trivial functions should help
make the transition between the two models clear (instead of just having some
unexplained -1's and +1's in the middle of the code.
**/
...
inline Node::size_type Node::degree() const { return neighbors().size(); }
...
inline Graph::size_type Graph::num_edges() const { return _num_edges; }
